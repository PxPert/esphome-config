...
void APIConnection::on_voice_assistant_response(const VoiceAssistantResponse &msg) {
  if (voice_assistant::global_voice_assistant != nullptr) {
   ...
   sockaddr_in sig_addr;
   inet_pton(AF_INET, "192.168.1.55", &sig_addr.sin_addr);
   sig_addr.sin_family = AF_INET;
   sig_addr.sin_port = htons(10000);

   std::unique_ptr<socket::Socket> sig_socket  = socket::socket(AF_INET, SOCK_DGRAM, IPPROTO_IP);
    if (sig_socket == nullptr) {
      ESP_LOGW(TAG, "Could not create SIG socket");
    }

   int enable = 1;
   int err = sig_socket->setsockopt(SOL_SOCKET, SO_REUSEADDR, &enable, sizeof(int));
  if (err != 0) {
    ESP_LOGW(TAG, "Socket unable to set reuseaddr: errno %d", err);
    // we can still continue
  }

   err = sig_socket->setblocking(true);
  if (err != 0) {
    ESP_LOGW(TAG, "Socket unable to set nonblocking mode: errno %d", err);
  }


    ESP_LOGW(TAG, "SOCKET SENDING %d", msg.port);
    int32_t conv = htonl(msg.port);
    char *data = (char*)&conv;
   sig_socket->sendto(data, sizeof(conv), 0, (struct sockaddr *) &sig_addr, sizeof(sig_addr));
    ESP_LOGW(TAG, "SOCKET SENT");


    struct sockaddr_storage storage;
    sockaddr_in* storage_sock = (struct sockaddr_in*) &storage;
//    socklen_t len = sizeof(storage);
//    this->helper_->getpeername((struct sockaddr *) &storage, &len);
    inet_pton(AF_INET, "192.168.1.55", &storage_sock->sin_addr);
